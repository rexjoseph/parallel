...
const PromptForm: FC<PromptProps> = ({ apiKey }) => {
  ...
  const handlePhoneSubmit: (e: FormEvent<HTMLFormElement>) => void = async (
    ...
    try {
      ...
      let phoneLogNew = [...phoneLog, prompt];
      ...
      await setPhoneLog([...phoneLogNew, res.data.data]);
  ...
  const handlePromptSubmit: (e: FormEvent<HTMLFormElement>) => void = async (
    ...
    try {
      ...
      await setChatLog([...chatLogNew, res.data.data.content]);
      await setPromptResReceived(res.data.data.content);
